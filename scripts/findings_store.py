...
class FindingsStore:
    ...
    def is_regression(self, fingerprint: str) -> bool:
        """Check whether a finding was previously fixed and has reappeared.

        A regression means the finding has a verified fix recorded in
        ``fix_history`` but its current status is no longer ``'fixed'``.
        This avoids relying on the transient status changes performed by
        ``record_scan()`` when a regression is detected.
