...
if [ ! -f "${DEBIAN_ARCHIVE}" ]; then
    echo "ERROR: Required repository Debian image is missing:"
    echo "${DEBIAN_ARCHIVE}"
    echo "NEXORA_DEBIAN_REPOSITORY_IMAGE_REQUIRED"
    exit 1
fi

echo "Using repository Debian base image ONLY: ${DEBIAN_ARCHIVE}"
