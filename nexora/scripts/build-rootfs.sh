...
if [ -f "${DEBIAN_ARCHIVE}" ]; then
    ...
    sudo debootstrap --arch=amd64 --variant=minbase --include=systemd,systemd-sysv,docker.io,containerd,runc,kmod,build-essential,pkg-config,git,curl,ca-certificates,unzip,xz-utils,python3,python3-pip,python3-venv,python3-dev,libffi-dev,libssl-dev,libsqlite3-dev,libpq-dev,iproute2,iptables "${DEBIAN_SUITE}" "${ROOTFS_DIR}" "${DEBIAN_MIRROR}"
fi
...
TERMUX_DEBIAN_PACKAGES=(gpgv bash bc bison clang cpio curl dos2unix ffmpeg file fish flex gawk git jq less lld llvm lsof lua5.4 m4 make nano net-tools nmap openssh-client openssh-server openssl parallel patch perl php-cli pkg-config postgresql-client procps psmisc qemu-system-x86 qemu-utils redis-server ripgrep rsync ruby gh tshark qemu-user rustc cargo screen sed strace sudo tar tmux tor tree unzip util-linux vim-nox w3m wget xorriso zip cmake ninja-build docker.io containerd runc python3 python3-pip python3-venv python3-dev p7zip-full)
...
declare -A TERMUX_COMMAND_PACKAGES=(
    ...
    [curl]=curl [ffmpeg]=ffmpeg [file]=file [fish]=fish [flex]=flex [gawk]=gawk [gh]=gh [git]=git [jq]=jq [lsof]=lsof
    [lua5.4]=lua5.4 [make]=make [nano]=nano [nmap]=nmap [ssh]=openssh-client [openssl]=openssl [parallel]=parallel [patch]=patch
    [perl]=perl [php]=php-cli [psql]=postgresql-client [qemu-system-x86_64]=qemu-system-x86 [redis-server]=redis-server
    [rg]=ripgrep [rsync]=rsync [ruby]=ruby [rustc]=rustc [cargo]=cargo [strace]=strace [sudo]=sudo [tar]=tar [tmux]=tmux
    [tor]=tor [tree]=tree [unzip]=unzip [vim]=vim-nox [w3m]=w3m [wget]=wget [xorriso]=xorriso [zip]=zip [cmake]=cmake
    [ninja]=ninja-build [tshark]=tshark
)
TERMUX_REQUIRED_COMMANDS=(7z bash bc clang cpio curl ffmpeg file fish flex gawk gh git jq lsof lua5.4 make nano nmap ssh openssl parallel patch perl php psql qemu-system-x86_64 redis-server rg rsync ruby rustc cargo strace sudo tar tmux tor tree unzip vim w3m wget xorriso zip cmake ninja tshark)

echo "=== Preflight Debian package/command mappings ==="
PREFLIGHT_FAILED=0
for cmd in "${TERMUX_REQUIRED_COMMANDS[@]}"; do
    package="${TERMUX_COMMAND_PACKAGES[$cmd]:-}"
    if [ -z "${package}" ]; then echo "ERROR: no Debian package mapping declared for command: ${cmd}"; PREFLIGHT_FAILED=1; continue; fi
    if ! printf '%s\n' "${TERMUX_DEBIAN_PACKAGES[@]}" | grep -Fxq "${package}"; then echo "ERROR: mapped package is not installed by the package list: ${cmd} -> ${package}"; PREFLIGHT_FAILED=1; fi
done
if [ "${PREFLIGHT_FAILED}" -ne 0 ]; then echo "ERROR: NEXORA Debian command mapping preflight failed."; exit 1; fi
echo "NEXORA_TERMUX_MAPPING_PREFLIGHT_OK"

DNS_SERVERS=""
for resolver_file in /run/systemd/resolve/resolv.conf /run/NetworkManager/resolv.conf; do
    if [ -f "${resolver_file}" ]; then DNS_SERVERS="$(awk '/^[[:space:]]*nameserver[[:space:]]+/ && $2 !~ /^(127\.|::1$)/ {print $2}' "${resolver_file}" | tr '\n' ' ' || true)"; [ -n "${DNS_SERVERS}" ] && break; fi
done
if [ -z "${DNS_SERVERS}" ] && command -v resolvectl >/dev/null 2>&1; then DNS_SERVERS="$(resolvectl dns 2>/dev/null | awk '{for (i=2; i<=NF; i++) if ($i !~ /^(127\.|::1$)/) print $i}' | tr '\n' ' ' || true)"; fi
if [ -z "${DNS_SERVERS}" ] && [ -f /etc/resolv.conf ]; then DNS_SERVERS="$(awk '/^[[:space:]]*nameserver[[:space:]]+/ && $2 !~ /^(127\.|::1$)/ {print $2}' /etc/resolv.conf | tr '\n' ' ' || true)"; fi
if [ -z "${DNS_SERVERS}" ]; then DNS_SERVERS="1.1.1.1 8.8.8.8"; fi
rm -f "${ROOTFS_DIR}/etc/resolv.conf"
for dns_server in ${DNS_SERVERS}; do printf "nameserver %s\n" "${dns_server}" >> "${ROOTFS_DIR}/etc/resolv.conf"; done
if grep -Eq "^[[:space:]]*nameserver[[:space:]]+(127\.|::1)" "${ROOTFS_DIR}/etc/resolv.conf" 2>/dev/null; then echo "ERROR: generated Debian chroot resolv.conf still contains a loopback nameserver."; cat "${ROOTFS_DIR}/etc/resolv.conf" || true; exit 1; fi
if ! chroot "${ROOTFS_DIR}" getent hosts deb.debian.org >/dev/null 2>&1; then echo "ERROR: Debian chroot DNS resolution failed before apt-get update."; cat "${ROOTFS_DIR}/etc/resolv.conf" || true; exit 1; fi
...
for cmd in "${TERMUX_REQUIRED_COMMANDS[@]}"; do
    if ! chroot "${ROOTFS_DIR}" sh -c "command -v '$cmd' >/dev/null 2>&1"; then echo "ERROR: mirrored Termux command is missing in NEXORA: $cmd"; exit 1; fi
done
echo "NEXORA_TERMUX_COMMANDS_OK"
if [ -x "${ROOTFS_DIR}/usr/bin/fdfind" ] && [ ! -e "${ROOTFS_DIR}/usr/local/bin/fd" ]; then ln -s /usr/bin/fdfind "${ROOTFS_DIR}/usr/local/bin/fd"; fi
...
(cd /tmp && grep " ${NODE_TARBALL}$" "${NODE_SUMS}" | sha256sum -c -)
...
if [ -x "${ROOTFS_DIR}/usr/local/lib/nodejs/node-v${NODE_VERSION}-linux-x64/bin/corepack" ]; then ln -sfn "/usr/local/lib/nodejs/node-v${NODE_VERSION}-linux-x64/bin/corepack" "${ROOTFS_DIR}/usr/local/bin/corepack"; fi
...
# Node.js ships npm and npx in its own installation. npm@11's global install
# also tries to own the /usr/local/bin/npm and /usr/local/bin/npx launchers.
# Treat those bundled tools as already satisfied and install only the
# additional manifest tools. This avoids EEXIST while preserving the pinned
# Node.js/npm toolchain.
NPM_GLOBAL_INSTALL_PACKAGES=()
for npm_package in "${NPM_GLOBAL_PACKAGES[@]}"; do
    case "${npm_package}" in
        npm@*|corepack@*) continue ;;
        *) NPM_GLOBAL_INSTALL_PACKAGES+=("${npm_package}") ;;
    esac
done
if [ "${#NPM_GLOBAL_INSTALL_PACKAGES[@]}" -gt 0 ]; then
    chroot "${ROOTFS_DIR}" /usr/local/bin/npm install --global --no-fund --no-audit "${NPM_GLOBAL_INSTALL_PACKAGES[@]}"
fi
for npm_package in "${NPM_GLOBAL_PACKAGES[@]}"; do
...
if ! chroot "${ROOTFS_DIR}" /usr/bin/docker compose version >/dev/null 2>&1; then echo "ERROR: Docker Compose v2 plugin is not available."; chroot "${ROOTFS_DIR}" /usr/bin/docker version || true; exit 1; fi
chroot "${ROOTFS_DIR}" /usr/bin/docker compose version
if ! chroot "${ROOTFS_DIR}" /usr/local/bin/docker-compose version >/dev/null 2>&1; then echo "ERROR: docker-compose compatibility wrapper is not available."; exit 1; fi
...
rm -f "${BUILDX_TMP}"
if ! chroot "${ROOTFS_DIR}" /usr/bin/docker buildx version >/dev/null 2>&1; then echo "ERROR: Docker Buildx plugin is not available."; chroot "${ROOTFS_DIR}" /usr/bin/docker version || true; exit 1; fi
chroot "${ROOTFS_DIR}" /usr/bin/docker buildx version
echo "NEXORA_DOCKER_BUILDX_OK"

echo "=== Verify Debian init ==="
if [ -x "${ROOTFS_DIR}/sbin/init" ]; then echo "NEXORA_DEBIAN_INIT_OK"; else echo "ERROR: Debian /sbin/init was not created."; ls -lah "${ROOTFS_DIR}/sbin" || true; exit 1; fi
echo "=== Verify systemd ==="
if [ -x "${ROOTFS_DIR}/lib/systemd/systemd" ]; then echo "NEXORA_SYSTEMD_OK"; else echo "ERROR: Debian systemd was not created."; exit 1; fi
if [ "$(readlink "${ROOTFS_DIR}/sbin/init" 2>/dev/null || true)" = "/lib/systemd/systemd" ]; then echo "NEXORA_INIT_SYSTEMD_OK"; else echo "WARNING: /sbin/init is not linked directly to /lib/systemd/systemd"; readlink "${ROOTFS_DIR}/sbin/init" || true; fi
...
if ! chroot "${ROOTFS_DIR}" /usr/local/bin/node --version | grep -q "^v${NODE_VERSION%%.*}\."; then echo "ERROR: Node.js ${NODE_VERSION} was not installed correctly."; chroot "${ROOTFS_DIR}" /usr/local/bin/node --version || true; exit 1; fi
if ! chroot "${ROOTFS_DIR}" /usr/local/bin/npm --version >/dev/null 2>&1; then echo "ERROR: npm was not installed correctly."; exit 1; fi
...
if [ -x "${ROOTFS_DIR}/usr/sbin/iptables-legacy" ]; then sudo ln -sfn /usr/sbin/iptables-legacy "${ROOTFS_DIR}/usr/sbin/iptables"; else echo "ERROR: iptables-legacy is missing from the NEXORA rootfs."; exit 1; fi
if [ -x "${ROOTFS_DIR}/usr/sbin/ip6tables-legacy" ]; then sudo ln -sfn /usr/sbin/ip6tables-legacy "${ROOTFS_DIR}/usr/sbin/ip6tables"; else echo "ERROR: ip6tables-legacy is missing from the NEXORA rootfs."; exit 1; fi
...
if [ -f "${IMAGE_TAR}" ]; then echo "Loading bundled Docker test image..." > /dev/console; docker load -i "${IMAGE_TAR}" > /dev/console 2>&1; else echo "Bundled Docker image not present; pulling ${IMAGE}..." > /dev/console; if ! docker pull "${IMAGE}" > /dev/console 2>&1; then echo "NEXORA_DOCKER_CONTAINER_FAILED: unable to load or pull ${IMAGE}" > /dev/console; exit 1; fi; fi
...
WORKSPACE
mkdir -p "${ROOTFS_DIR}/var/lib/docker" "${ROOTFS_DIR}/var/lib/containerd" "${ROOTFS_DIR}/persist"
...
if ! systemctl start containerd; then log "NEXORA_CONTAINERD_FAILED"; systemctl status containerd --no-pager -l > "$CONSOLE" 2>&1 || true; journalctl -u containerd -n 80 --no-pager > "$CONSOLE" 2>&1 || true; exit 1; fi
if ! systemctl is-active --quiet containerd; then log "NEXORA_CONTAINERD_FAILED"; systemctl status containerd --no-pager -l > "$CONSOLE" 2>&1 || true; journalctl -u containerd -n 80 --no-pager > "$CONSOLE" 2>&1 || true; exit 1; fi
log "NEXORA_CONTAINERD_OK"
log "=== START DOCKER ==="
if ! systemctl start docker; then log "NEXORA_DOCKER_FAILED"; systemctl status docker --no-pager -l > "$CONSOLE" 2>&1 || true; journalctl -u docker -n 80 --no-pager > "$CONSOLE" 2>&1 || true; exit 1; fi
if ! systemctl is-active --quiet docker || ! docker info >/dev/null 2>&1; then log "NEXORA_DOCKER_FAILED"; systemctl status docker --no-pager -l > "$CONSOLE" 2>&1 || true; journalctl -u docker -n 80 --no-pager > "$CONSOLE" 2>&1 || true; docker info > "$CONSOLE" 2>&1 || true; exit 1; fi
...
if [ "${DOCKER_DRIVER}" != "overlay2" ]; then log "NEXORA_DOCKER_RUNTIME_FAILED: expected overlay2 storage driver"; docker info > "${CONSOLE}" 2>&1 || true; exit 1; fi
if [ "${DOCKER_CGROUP_VERSION}" != "2" ]; then log "NEXORA_DOCKER_RUNTIME_FAILED: expected cgroup v2"; docker info > "${CONSOLE}" 2>&1 || true; exit 1; fi
case "${DOCKER_SECURITY_OPTIONS}" in *seccomp*) ;; *) log "NEXORA_DOCKER_RUNTIME_FAILED: seccomp is not active"; docker info > "${CONSOLE}" 2>&1 || true; exit 1 ;; esac
NETWORK_NAME="nexora-runtime-smoke"
docker network rm "${NETWORK_NAME}" >/dev/null 2>&1 || true
if ! docker network create "${NETWORK_NAME}" >/dev/null 2>&1; then log "NEXORA_DOCKER_NETWORK_FAILED"; docker network ls > "${CONSOLE}" 2>&1 || true; exit 1; fi
docker network inspect "${NETWORK_NAME}" > /dev/null
docker network rm "${NETWORK_NAME}" >/dev/null
log "NEXORA_DOCKER_NETWORK_OK"
if ! docker compose version > "${CONSOLE}" 2>&1; then log "NEXORA_DOCKER_COMPOSE_FAILED"; exit 1; fi
...
#!/bin/sh
set -eu
fail() { echo "NEXORA_DEBIAN_ISOLATION_FAILED: $*" > /dev/console; exit 1; }
...
for p in /data/data/com.termux /data/data /system /system_ext /vendor /apex; do if [ -e "$p" ]; then fail "Android/Termux host path is visible: $p"; fi; done
if grep -Eiq '(^|[[:space:]])/data/data/com\.termux([[:space:]]|/)|(^|[[:space:]])/data/data([[:space:]]|/)' /proc/self/mountinfo 2>/dev/null; then fail "Android/Termux path appears in guest mount namespace"; fi
if env | grep -Eiq '(^|=)(TERMUX|TERMUX_VERSION|PREFIX=/data/data/com\.termux|HOME=/data/data/com\.termux)'; then fail "Termux environment leaked into Debian"; fi
for cmd in sh ps mount cat; do path="$(command -v "$cmd" 2>/dev/null || true)"; [ -n "$path" ] || fail "required Debian command missing: $cmd"; case "$path" in /data/*|/system/*|/apex/*) fail "Android executable resolved for $cmd: $path" ;; esac; done
