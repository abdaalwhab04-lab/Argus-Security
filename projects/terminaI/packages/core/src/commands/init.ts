...
export function performInit(doesGeminiMdExist: boolean): CommandActionReturn {
  if (doesGeminiMdExist) {
    return {
      ...
      messageType: 'info',
      content:
        'A terminaI.md file already exists in this directory. No changes were made.',
    };
  }
  ...
  return {
    type: 'submit_prompt',
    content: `
You are an AI agent that brings the power of Gemini directly into the terminal. Your task is to analyze the current directory and generate a comprehensive terminaI.md file to be used as instructional context for future interactions.
...
**terminaI.md Content Generation:**
...
Write the complete content to the \`terminaI.md\` file. The output must be well-formatted Markdown.
`,
  };
}
